...
import (
	...
	nodelabsante "github.com/nodelabs-sdk/nodelabs/app/ante"
	appconfig "github.com/nodelabs-sdk/nodelabs/config"
	"github.com/nodelabs-sdk/nodelabs/docs"
...
const appName = "nodelabsd"

// defaultNodeHome default home directories for the application daemon
var defaultNodeHome string

var (
	_ runtime.AppI                = (*NodelabsApp)(nil)
	_ cosmosevmserver.Application = (*NodelabsApp)(nil)
	_ ibctesting.TestingApp       = (*NodelabsApp)(nil)
)

// NodelabsApp extends an ABCI application, but with most of its parameters exported.
type NodelabsApp struct {
	*baseapp.BaseApp
	...
	// EVMMempool is the Krakatoa app-side EVM mempool (see mempool.go).
	// Required whenever the JSON-RPC server runs; nil when disabled via
	// mempool.max-txs=-1, in which case CometBFT's stock mempool is used.
...
// NewApp returns a reference to an initialized NodelabsApp.
func NewApp(
	logger log.Logger,
	db dbm.DB,
	loadLatest bool,
	...
	app.AccountKeeper = authkeeper.NewAccountKeeper(
		...
		authtypes.ProtoBaseAccount, appconfig.GetMaccPerms(),
	...
	// The module owner is a parameter and its access grants live in the
	// module's own store.
	...
	// NetworkKeeper consumes the license keeper: activation limits count a
	// holder's active licenses. Its own owner is a parameter and its access
	// grants live in its own store.
	...
	// NOTE: bank module must go first (SDK v0.54 requirement); the feemarket
	// module should go last of the end blockers that actually do something,
	...
	genesisModuleOrder := []string{
		...
		// Custom modules. Each owns its access grants, so the only ordering
		// constraint is that network follows license: it consumes the license
		// keeper.
	...
	// Wire the Krakatoa app-side EVM mempool (required for the JSON-RPC
	// server in cosmos/evm v0.7+). Must come after setAnteHandler — see
	// configureEVMMempool.
	...
	if loadLatest {
		...
		// set global evm variables from KV store so evmCoinInfo is populated
		// before any RPC handler runs
		vmModule.HydrateGlobals(ctx)
	}

	// Wire the sequential EVM tx runner. The wrapper installs the EVM module's
	// PatchTxResponses post-execution log/tx-index fix-up; swapping the inner
	// runner for txnrunner.NewSTMRunner would opt into BlockSTM, which nodelabs
...
func (app *NodelabsApp) setAnteHandler(txConfig client.TxConfig, maxGasWanted uint64) {
	...
	options := nodelabsante.HandlerOptions{
		Cdc:                    app.appCodec,
	...
	app.SetAnteHandler(nodelabsante.NewAnteHandler(options))
}

func (app *NodelabsApp) onPendingTx(hash common.Hash) {
...
func (app *NodelabsApp) RegisterPendingTxListener(listener func(common.Hash)) {
	app.pendingTxListeners = append(app.pendingTxListeners, listener)
}

func (app *NodelabsApp) setPostHandler() {
...
func (app *NodelabsApp) Name() string { return app.BaseApp.Name() }

// BeginBlocker application updates every begin block
func (app *NodelabsApp) BeginBlocker(ctx sdk.Context) (sdk.BeginBlock, error) {
	return app.ModuleManager.BeginBlock(ctx)
}

// EndBlocker application updates every end block
func (app *NodelabsApp) EndBlocker(ctx sdk.Context) (sdk.EndBlock, error) {
	return app.ModuleManager.EndBlock(ctx)
}

func (app *NodelabsApp) FinalizeBlock(req *abci.RequestFinalizeBlock) (res *abci.ResponseFinalizeBlock, err error) {
	return app.BaseApp.FinalizeBlock(req)
}

func (app *NodelabsApp) Configurator() module.Configurator {
	return app.configurator
}

// InitChainer application update at chain initialization
func (app *NodelabsApp) InitChainer(ctx sdk.Context, req *abci.RequestInitChain) (*abci.ResponseInitChain, error) {
...
func (app *NodelabsApp) PreBlocker(ctx sdk.Context, _ *abci.RequestFinalizeBlock) (*sdk.ResponsePreBlock, error) {
	return app.ModuleManager.PreBlock(ctx)
}

// LoadHeight loads a particular height
func (app *NodelabsApp) LoadHeight(height int64) error {
	return app.LoadVersion(height)
}

// LegacyAmino returns the app's amino codec.
func (app *NodelabsApp) LegacyAmino() *codec.LegacyAmino {
	return app.legacyAmino
}

// AppCodec returns the app's codec.
func (app *NodelabsApp) AppCodec() codec.Codec {
	return app.appCodec
}

// InterfaceRegistry returns the app's InterfaceRegistry
func (app *NodelabsApp) InterfaceRegistry() types.InterfaceRegistry {
	return app.interfaceRegistry
}

// TxConfig returns the app's TxConfig
func (app *NodelabsApp) TxConfig() client.TxConfig {
	return app.txConfig
}

// DefaultGenesis returns a default genesis from the registered AppModuleBasic's.
func (app *NodelabsApp) DefaultGenesis() map[string]json.RawMessage {
...
func (app *NodelabsApp) GetKey(storeKey string) *storetypes.KVStoreKey {
	return app.keys[storeKey]
}

// SimulationManager implements the SimulationApp interface
func (app *NodelabsApp) SimulationManager() *module.SimulationManager {
	return app.sm
}

// RegisterAPIRoutes registers all application module routes with the provided API server.
func (app *NodelabsApp) RegisterAPIRoutes(apiSvr *api.Server, apiConfig config.APIConfig) {
...
func (app *NodelabsApp) RegisterTxService(clientCtx client.Context) {
	authtx.RegisterTxService(app.GRPCQueryRouter(), clientCtx, app.Simulate, app.interfaceRegistry)
}

// RegisterTendermintService implements the Application.RegisterTendermintService method.
func (app *NodelabsApp) RegisterTendermintService(clientCtx client.Context) {
...
func (app *NodelabsApp) RegisterNodeService(clientCtx client.Context, cfg config.Config) {
...
func (app *NodelabsApp) GetBaseApp() *baseapp.BaseApp {
	return app.BaseApp
}

func (app *NodelabsApp) GetStakingKeeperSDK() stakingkeeper.Keeper {
	return *app.StakingKeeper
}

func (app *NodelabsApp) GetIBCKeeper() *ibckeeper.Keeper {
	return app.IBCKeeper
}

func (app *NodelabsApp) GetEVMKeeper() *evmkeeper.Keeper {
	return app.EVMKeeper
}

func (app *NodelabsApp) GetErc20Keeper() *erc20keeper.Keeper {
	return &app.Erc20Keeper
}

func (app *NodelabsApp) SetErc20Keeper(erc20Keeper erc20keeper.Keeper) {
	app.Erc20Keeper = erc20Keeper
}

func (app *NodelabsApp) GetGovKeeper() govkeeper.Keeper {
	return app.GovKeeper
}

func (app *NodelabsApp) GetEvidenceKeeper() *evidencekeeper.Keeper {
	return &app.EvidenceKeeper
}

func (app *NodelabsApp) GetSlashingKeeper() slashingkeeper.Keeper {
	return app.SlashingKeeper
}

func (app *NodelabsApp) GetBankKeeper() bankkeeper.Keeper {
	return app.BankKeeper
}

func (app *NodelabsApp) GetFeeMarketKeeper() *feemarketkeeper.Keeper {
	return &app.FeeMarketKeeper
}

func (app *NodelabsApp) GetFeeGrantKeeper() feegrantkeeper.Keeper {
	return app.FeeGrantKeeper
}

func (app *NodelabsApp) GetConsensusParamsKeeper() consensusparamkeeper.Keeper {
	return app.ConsensusParamsKeeper
}

func (app *NodelabsApp) GetAccountKeeper() authkeeper.AccountKeeper {
	return app.AccountKeeper
}

func (app *NodelabsApp) GetAuthzKeeper() authzkeeper.Keeper {
	return app.AuthzKeeper
}

func (app *NodelabsApp) GetDistrKeeper() distrkeeper.Keeper {
	return app.DistrKeeper
}

func (app *NodelabsApp) GetStakingKeeper() *stakingkeeper.Keeper {
	return app.StakingKeeper
}

func (app *NodelabsApp) GetMintKeeper() mintkeeper.Keeper {
	return app.MintKeeper
}

func (app *NodelabsApp) GetCallbackKeeper() ibccallbackskeeper.ContractKeeper {
	return app.CallbackKeeper
}

func (app *NodelabsApp) GetTransferKeeper() *transferkeeper.Keeper {
	return app.TransferKeeper
}

func (app *NodelabsApp) SetTransferKeeper(transferKeeper *transferkeeper.Keeper) {
	app.TransferKeeper = transferKeeper
}

func (app *NodelabsApp) GetMempool() sdkmempool.ExtMempool {
	return app.EVMMempool
}

func (app *NodelabsApp) GetAnteHandler() sdk.AnteHandler {
	return app.BaseApp.AnteHandler()
}

func (app *NodelabsApp) GetTxConfig() client.TxConfig {
	return app.txConfig
}

// Close shuts down the mempool (if wired) and the underlying BaseApp.
func (app *NodelabsApp) Close() error {
	var err error
...
func (app *NodelabsApp) AutoCliOpts() autocli.AppOptions {
